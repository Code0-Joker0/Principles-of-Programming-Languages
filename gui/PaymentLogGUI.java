...
public class PaymentLogGUI extends JFrame {
    ...
    private int selectedTxnId() {
        Object selected = cbTransaction.getSelectedItem();
        if (selected == null) throw new IllegalStateException("No transaction selected.");
        return Integer.parseInt(((String) selected).split(" - ")[0]);
    }

    private void addLog() {
        if (cbTransaction.getItemCount() == 0) {
            JOptionPane.showMessageDialog(this, "No transactions available to log.");
            return;
        }
