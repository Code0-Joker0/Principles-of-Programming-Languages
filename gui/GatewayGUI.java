...
public class GatewayGUI extends JPanel {
    ...
    private JPanel buildHeader() {
        ...
        tfSearch.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            public void insertUpdate(javax.swing.event.DocumentEvent e)  { loadGateways(tfSearch.getText()); }
            public void removeUpdate(javax.swing.event.DocumentEvent e)  { loadGateways(tfSearch.getText()); }
            public void changedUpdate(javax.swing.event.DocumentEvent e) { loadGateways(tfSearch.getText()); }
        });
    ...
    private JPanel buildForm() {
        ...
        btnSave.addActionListener(e -> saveGateway());
        ...
        formPanel.add(fl("Gateway Name")); formPanel.add(Box.createVerticalStrut(4)); formPanel.add(tfName);
        formPanel.add(Box.createVerticalStrut(12));
        formPanel.add(fl("API Endpoint")); formPanel.add(Box.createVerticalStrut(4)); formPanel.add(tfEndpoint);
        formPanel.add(Box.createVerticalStrut(12));
    ...
    private void loadGateways(String search) {
        model.setRowCount(0);
    ...
    private void saveGateway() {
        ...
        try (Connection c = DBConnection.getConnection()) {
            if (editingId < 0) {
                ...
                ps.executeUpdate();
                logAudit(c, "gateways", "INSERT", -1, "Added gateway: " + name);
            } else {
                PreparedStatement ps = c.prepareStatement(
                ...
                ps.executeUpdate();
                logAudit(c, "gateways", "UPDATE", editingId, "Updated gateway: " + name);
            }
        } catch (SQLException e) { showErr(e); return; }
        loadGateways(tfSearch.getText());
        closeForm();
    ...
    private void deleteGateway() {
        ...
        try (Connection c = DBConnection.getConnection();
            ...
            ps.executeUpdate();
            logAudit(c, "gateways", "DELETE", editingId, "Deleted gateway #" + editingId);
        } catch (SQLException e) { showErr(e); return; }
        loadGateways(tfSearch.getText());
        closeForm();
        dashboard.refreshStats();
