...
public class TransactionGUI extends JFrame {
    ...
    private int selectedId(JComboBox<String> cb) {
        ...
        if (selected == null) throw new IllegalStateException("No item selected in dropdown.");
        return Integer.parseInt(((String) selected).split(" - ")[0]);
    }

    private void addTransaction() {
        if (cbUser.getItemCount() == 0 || cbGateway.getItemCount() == 0) {
            JOptionPane.showMessageDialog(this, "No users or gateways available. Please add them first.");
            return;
        }
