...
public class MainDashboard extends JFrame {
    ...
    private JPanel buildSidebar() {
        ...
        JLabel logoLabel = new JLabel("\uD83D\uDCB8 TxnManager");
        ...
        section.setBorder(BorderFactory.createEmptyBorder(10, 0, 6, 0));
        section.setMaximumSize(new Dimension(210, 28));
        ...
        JLabel version = new JLabel("  DBMS Project", SwingConstants.LEFT);
    ...
    public void showCard(String key) { cardLayout.show(contentPanel, key); }
    ...
    private JPanel buildQuickActions() {
        ...
        JButton b3 = Theme.ghostButton("Payment Logs");
        ...
        new SwingWorker<int[], Void>() {
            @Override protected int[] doInBackground() throws Exception {
                ...
                try (Connection c = DBConnection.getConnection();
                    ...
                    if (rs.next()) d[0] = rs.getInt(1);
                    rs = st.executeQuery("SELECT COUNT(*) FROM transactions");
                    if (rs.next()) d[1] = rs.getInt(1);
                    rs = st.executeQuery("SELECT COALESCE(SUM(amount),0) FROM transactions WHERE status='SUCCESS'");
                    if (rs.next()) d[2] = rs.getInt(1);
                    rs = st.executeQuery("SELECT COUNT(*) FROM gateways WHERE is_active=1");
                    if (rs.next()) d[3] = rs.getInt(1);
                }
                return d;
            }
