...
public class TransactionManager {
    ...
    // Absolute path relative to the JAR/class location to avoid working-directory issues
    private static final String FILE_PATH = Paths.get(
            System.getProperty("user.home"), "transactions.txt").toString();
    ...
    // Returns the highest transaction ID currently loaded, or 0 if none
    public synchronized int getMaxId() {
        return transactions.keySet().stream().mapToInt(i -> i).max().orElse(0);
    }
    ...
    public synchronized void loadFromFile() {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(file))) {
            ...
            while ((line = br.readLine()) != null) {
                String[] parts = line.split(",", 6); // limit=6 to preserve timestamp with spaces
                ...
                t.setTimestamp(parts[5].trim()); // restore original timestamp
